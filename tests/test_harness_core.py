...
from scripts.run_evaluation import load_queries
...
def test_load_queries_from_dataset(tmp_path: Path) -> None:
    dataset = tmp_path / "dataset.jsonl"
    dataset.write_text(
        '{"query":"q1","expected":"e1"}\n{"query":"q2","expected":"e2"}\n',
        encoding="utf-8",
    )
    rows = load_queries(str(dataset))
    assert rows == [("q1", "e1"), ("q2", "e2")]
